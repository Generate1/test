  ...
  */
  if (_STR_EQ(n->name, "program")) {
    ...
    */
    ...
    // 1.1 flatten params
    auto parmas_node = n->children[3];
    if (!_STR_EQ(parmas_node->children[0]->name, "void")) {
      std::stack<syntax_tree_node *> param_stack;
      auto param_list_ptr = parmas_node->children[0];

      while (param_list_ptr->children_num == 3) {
        param_stack.push(param_list_ptr->children[2]); 
        param_list_ptr = param_list_ptr->children[0];   
      }
      param_stack.push(param_list_ptr->children[0]);

      while (!param_stack.empty()) {
        auto param = static_cast<ASTParam *>(transform_node_iter(param_stack.top()));
        node->params.push_back(std::shared_ptr<ASTParam>(param));
        param_stack.pop();
      }
    }
    // 1.2 compound_stmt 字段填充
    auto compound_stmt = static_cast<ASTCompoundStmt *>(transform_node_iter(n->children[5]));
    node->compound_stmt = std::shared_ptr<ASTCompoundStmt>(compound_stmt);
    ...
    */
    ...
    auto local_decl_node = n->children[1];
    
    if (local_decl_node->children_num > 0 && !_STR_EQ(local_decl_node->children[0]->name, "epsilon")) {
      std::stack<syntax_tree_node *> local_stack;
      auto local_ptr = local_decl_node;
      while (local_ptr->children_num == 2) {
        local_stack.push(local_ptr->children[1]);  
        local_ptr = local_ptr->children[0];    
      }



      while (!local_stack.empty()) {
        auto var_decl = static_cast<ASTVarDeclaration *>(transform_node_iter(local_stack.top()));
        node->local_declarations.push_back(std::shared_ptr<ASTVarDeclaration>(var_decl));
        local_stack.pop();
      }
    }
    // statement list
    // 2.2 flatten statement-list
    auto stmt_list_node = n->children[2];

    if (stmt_list_node->children_num > 0 && !_STR_EQ(stmt_list_node->children[0]->name, "epsilon")) {
      std::stack<syntax_tree_node *> stmt_stack;
      auto stmt_ptr = stmt_list_node;


      while (stmt_ptr->children_num == 2) {
        stmt_stack.push(stmt_ptr->children[1]);
        stmt_ptr = stmt_ptr->children[0];   
      }
      ...
      while (!stmt_stack.empty()) {
        auto stmt = static_cast<ASTStatement *>(transform_node_iter(stmt_stack.top()));
        node->statement_list.push_back(std::shared_ptr<ASTStatement>(stmt));
        stmt_stack.pop();
      }
    }
    ...
    */
    // 5.1 expresstion
    auto expr = static_cast<ASTExpression *>(transform_node_iter(n->children[2]));
    node->expression = std::shared_ptr<ASTExpression>(expr);
    // 5.2 if statement
    auto if_stmt = static_cast<ASTStatement *>(transform_node_iter(n->children[4]));
    node->if_statement = std::shared_ptr<ASTStatement>(if_stmt);
    // check whether this selection statement contains
    // 5.3 else structure
    if (n->children_num == 7) {
      auto else_stmt = static_cast<ASTStatement *>(transform_node_iter(n->children[6]));
      node->else_statement = std::shared_ptr<ASTStatement>(else_stmt);
    } else {
    ...
    if (n->children_num == 3) {
      ...
      */
      ...
      auto add_expr = static_cast<ASTAdditiveExpression *>(transform_node_iter(n->children[0]));
      node->additive_expression = std::shared_ptr<ASTAdditiveExpression>(add_expr);
      ...
      auto term = static_cast<ASTTerm *>(transform_node_iter(n->children[2]));
      node->term = std::shared_ptr<ASTTerm>(term);
    } else {
      auto term_node =
    ...
    if (_STR_EQ(name, "expression") || _STR_EQ(name, "var") ||
      ...
      */
      if (_STR_EQ(name, "integer")) {
        ...
        // 3.2
        num_node->f_val = std::stof(n->children[i]->children[0]->name);  // 字符串转float
        num_node->type = TYPE_FLOAT;  // 标记为float类型
      } else {
        _AST_NODE_ERROR_
      }
      return num_node;
    ...
    node->id = n->children[0]->name;
    // flatten args
    if (!_STR_EQ(n->children[2]->children[0]->name, "epsilon") && _STR_EQ(n->children[2]->children[0]->name, "arg-list")) {
